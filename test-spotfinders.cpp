...
TEST(SpotFinder, simpleHistogramFinder) {
    ...
    rs::QuantileStats<float> res_y = residuals_simple.get2();
...
TEST(SpotFinder, ceresFitFinder) {
    std::mt19937_64 rng(0xBEEBBEEB);
    ...
    for (size_t ii = 0; ii < 5000; ++ii) {
        ...
        SpotFinder::make_test_img(test_img, solution, rng);
        cv::Vec2d const simple_solution = finder.ceresFitFinder(test_img);
        ...
        cv::Vec2d const diff = simple_solution - solution;
        residuals.push_unsafe(diff[0], diff[1]);
    }

    rs::QuantileStats<float> res_x = residuals.get1();
    rs::QuantileStats<float> res_y = residuals.get2();

    res_x.plotHistAndCDF("ceres-fit-finder-x-residuals",
                         rs::HistConfig()
                         .setDataLabel("Residual [px]"));
    ...
    residuals.plotHist("ceres-fit-finder-residuals",
                              {-1,-1},
                              rs::HistConfig()
                              .setTitle("Solution residuals")
                              .setXLabel("x [px]")
